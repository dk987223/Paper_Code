...
xi(6) = 0; 
for i=1:5  
    xi(6-i) = xi(6+1-i) + standstill_distance + p0*vi(6-i+1)*vi(6-i+1) + p1*vi(6-i+1);
end
xi = xi';
x0 = xi(1) + standstill_distance + p0*vi(1).*vi(1) + p1*vi(1);
...
% V0¼ÓËÙ¶ÈÑ¡Ôñ
 ...
 for i = 1:6
       ...
       if abs(max(ex0)) < 10e-8 &&  abs(min(ex0)) < 10e-8
           ...
           PL = (sign(min(ex0)) - xita1)*ro - Rinf * sign(min(ex0));
           DPL = (sign(min(ex0)) - xita1)*dro;
           PR = (sign(max(ex0)) + xita2)*ro - Rinf * sign(max(ex0));
           DPR = (sign(max(ex0)) - xita2)*dro;
       end

       
       va = (exi - PL)./(PR - PL);
       r1 = 1./((1-va).*(PR-PL).*va);
       b1 = (PL.*DPL - DPL.*PR - exi.*(DPR - DPL))./(PR - PL);
       wi = log(va./(1-va)); 
...
h(j) = exp(-norm(mmi-cij(:,j))^2/(2*bj^2));
       ...
       adp_wwwi = adp_wwwi + dwwwi*period;
         ...
         for i = 1:6
            ...
            if  i == 1
                u(6) = (W_rad/eff)*(mi*b1(6)./(2*p0*vi(6)+p1) + mi*(vi(5)-vi(6))./(2*p0*vi(6)+p1)+ mi*adp_wwwi(6)*tanh(Si(6)./0.1)- mi*adp_wwi(6)*h(6)+ adp_C(6).*vi(6).^2+ mi*g*adp_f(6) + mi*(k3*Si(6)/(norm(Si(6))+deta) + q*langda*wi(6))./(q*r1(6)*(2*p0*vi(6)+p1)));
            elseif i > 1 && i < 6
                u(7-i) = (W_rad/eff)*(mi*b1(7-i)./(2*p0*vi(7-i)+p1) + mi*(vi(7-1-i)-vi(7-i))./(2*p0*vi(7-i)+p1) + mi*adp_wwwi(7-i)*tanh(Si(7-i)./0.1) - mi*adp_wwi(7-i)*h(7-i)   +  adp_C(7-i).*vi(7-i).^2+ mi*g*adp_f(7-i)...
               ...
               u(1) = (W_rad/eff)*(mi*b1(1)./(2*p0*vi(1)+p1) + mi*(v0-vi(1))./(2*p0*vi(1)+p1) + mi*adp_wwwi(1)*tanh(Si(1)./0.1) - mi*adp_wwi(1)*h(1)+ adp_C(1).*vi(1).^2+ mi*g*adp_f(1)...
...
B=plot(t,SS1,t,SS2,t,SS3,t,SS4,t,SS5,t,SS5,t,SS6)
B(1).LineWidth = xiankuan;
B(2).LineWidth = xiankuan;
B(3).LineWidth = xiankuan;
B(4).LineWidth = xiankuan;
...
B(6).LineWidth = xiankuan;
B(7).LineWidth = xiankuan;
legend('$S_1$' ,'$S_2$','$S_3$','$S_4$','$S_5$','$S_6$','Interpreter', 'latex')
set(gca,'FontName','Times New Roman','FontSize',zihao,'FontWeight','normal')
xlabel('time (s)','FontSize',zihao)
set(gca,'FontName','Times New Roman','FontSize',zihao,'FontWeight','normal')
...
legend(	'$Z_{li}$','$e_1$' ,'$e_2$','$e_3$','$e_4$','$e_5$','$e_6$','$Z_{ri}$','Interpreter', 'latex')
set(gca,'FontName','Times New Roman','FontSize',zihao,'FontWeight','normal')
xlabel('time (s)','FontSize',zihao)
set(gca,'FontName','Times New Roman','FontSize',zihao,'FontWeight','normal')
